...
namespace BOOM{
  ...
  public:
    ...
    void set_prm(const Ptr<P> & p){prm_ = p; set_t();}
  ...
  template<class P>
  ParamPolicy_1<P>::ParamPolicy_1(const Ptr<P> & pPrm)
      : prm_(pPrm)
    {
      set_t();
    }
  ...
  template<class P>
  ParamVector ParamPolicy_1<P>::parameter_vector() {return t_;}

  template<class P>
  const ParamVector ParamPolicy_1<P>::parameter_vector() const {return t_;}

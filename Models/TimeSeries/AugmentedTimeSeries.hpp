...
namespace BOOM{
  ...
  public:
    ...
    AugmentedTimeSeries(const Ptr<FIRST> & x0, Ptr<D> proto, const string & ID="");
  ...
  AugmentedTimeSeries<D,F>::AugmentedTimeSeries(const Ptr<F> & first, const string &ID)
    : TimeSeries<D>(ID),
      x0_(first)
  {}

  template<class D, class F>
  AugmentedTimeSeries<D,F>::AugmentedTimeSeries(const Ptr<F> & first, Ptr<D> d, const string &ID)
    : TimeSeries<D>(d,ID),
      x0_(first)
  {}

  template<class D, class F>
  AugmentedTimeSeries<D,F>::AugmentedTimeSeries(const Ptr<F> & x0, const std::vector<Ptr<D> > &v,
  ...
  template <class D, class F>
  template <class FwdIt>
  AugmentedTimeSeries<D,F>::AugmentedTimeSeries(const Ptr<F> & first, FwdIt Beg, FwdIt End, bool reset_links,
...
//   }
  ...
  template <class D, class F>
  void AugmentedTimeSeries<D,F>::set_x0(const Ptr<F> & x){ x0_ = x;}

...
namespace BOOM{
  ...
  MD::MarkovData(uint val, const Ptr<CatKeyBase> &key)
    : CD(val, key)
  {
    clear_links();
  }

  MD::MarkovData(uint val, const Ptr<MarkovData> &last)
    : CD(val, last->key())
  {
    set_prev(last);
    last->set_next(this);
  ...
  SDO::StationaryDistObserver(const Ptr<VectorParams> &p)
    : stat(p)
  {}
  ...
  RowObserver::RowObserver(const Ptr<MatrixParams> &M, uint I)
    : mp(M),
      i(I)
  {
    m = mp->value();

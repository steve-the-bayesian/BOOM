...
namespace {
  ...
  TEST_F(HolidayMcmcTest, Mcmc) {
    ...
    // (1) Try things without the date range holiday.
    // (2) Then try things without the other holidays.
    //
    // If it does not matter which holidays are included then the problem is
    // with the model.

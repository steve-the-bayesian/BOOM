...
namespace BOOM {
  ...
  void MNP::set_Sigma(const SpdMatrix &S) {
    Sigma_prm()->set_var(S);
  }
  void MNP::set_siginv(const SpdMatrix &ivar) {
  ...
  void MNP::add_data(const Ptr<Data> &dp) {
    add_data(DAT(dp));
  }
  ...
  void MNP::impute_u(RNG &rng,
                     Vector &u,
                     const Ptr<ChoiceData> &dp,
                     TrunMvnTF & target) {
  ...
  void MNP::impute_u_slice(Vector &u, const Ptr<ChoiceData> &dp, TrunMvnTF & target) {
    // slice sampler
    eta(dp, wsp);
    target.set_mu(wsp);
    uint y = dp->value();

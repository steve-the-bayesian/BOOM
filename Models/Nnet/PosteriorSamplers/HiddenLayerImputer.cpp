...
namespace BOOM {
  ...
  HiddenLayerImputer::HiddenLayerImputer(const Ptr<HiddenLayer> &layer,
    ...
    {
      input_workspace_.resize(layer_->input_dimension());
    }
  ...
  void HiddenLayerImputer::impute_inputs(
    ...
    Nnet::to_numeric(outputs[layer_index_ - 1], input_workspace_);
    for (int i = 0; i < allocation_probs.size(); ++i) {
      complementary_allocation_probs[i] = log(1 - log(allocation_probs[i]));
      allocation_probs[i] = log(allocation_probs[i]);
    }
    double logp_current = input_full_conditional(
        input_workspace_, outputs[layer_index_],
        allocation_probs, complementary_allocation_probs);
    for (int i = 0; i < input_workspace_.size(); ++i) {
      input_workspace_[i] = 1 - input_workspace_[i];
      double logp_cand =
          input_full_conditional(input_workspace_, outputs[layer_index_],
                                 allocation_probs, complementary_allocation_probs);
      double logu = log(runif_mt(rng));
      if (logu < logp_cand - lse2(logp_cand, logp_current)) {
        logp_current = logp_cand;
      } else {
        input_workspace_[i] = 1 - input_workspace_[i];
      }
      outputs[layer_index_ - 1][i] = input_workspace_[i] > .5;
    }
  ...
  double HiddenLayerImputer::input_full_conditional(
    ...
    for (int input = 0; input < inputs.size(); ++input) {
      ans += input > .5 ? logp[input] : logp_complement[input];
    }
    return ans;
  }
  ...
  void HiddenLayerImputer::store_latent_data(Nnet::HiddenNodeValues &outputs) {
    ...
    Nnet::to_binary(input_workspace_, inputs);
  ...
  std::vector<Ptr<BinomialRegressionData>> HiddenLayerImputer::get_data_row(
    ...
    // Check if inputs exists in the passive data store add it to the active
    // data store, and return the answer.
    it = passive_data_store_.find(inputs);
    if (it != passive_data_store_.end()) {

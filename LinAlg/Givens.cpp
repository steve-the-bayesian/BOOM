...
namespace BOOM {
  ...
  void drot(int n, double *x, int x_stride, double *y, int y_stride, double c, double s) {
    if (n <= 0) return;
  ...
  Matrix & operator*(const GR &G, Matrix &A) {
    ...
    // EigenMap(A.row(G.i)).applyOnTheLeft(Eigen::JacobiRotation<double>(G.c, G.s));
    // EigenMap(A.row(G.j)).applyOnTheLeft(Eigen::JacobiRotation<double>(G.c, G.s));
    drot(x.size(), x.data(), x.stride(), y.data(), y.stride(), G.c, G.s);
    return A;
  }

  Matrix & operator*(Matrix &A, const GR &G){
    // EigenMap(A.col(G.i)).applyOnTheRight(Eigen::JacobiRotation<double>(G.c, -(G.s)));
    // EigenMap(A.col(G.j)).applyOnTheRight(Eigen::JacobiRotation<double>(G.c, -(G.s)));

...
def scan(fname, sep="", skip=0, nmax=-1, dtype=float, what=None):
    """
    Read the contents of a text file into a 1-d Numpy array.

    Args:
      sep:  The delimiter between fields.
      skip:  The number of initial lines of the file to skip.
      nmax: The maximum number of rows to read.  Note that this differs from R's
        nmax which is the maximum number of entries to read.
      dtype:  The data type of each read entry.  This is in place of R's 'what'
      what:  An alias for dtype.  If 'what' is given then 'dtype' is ignored.  
    """

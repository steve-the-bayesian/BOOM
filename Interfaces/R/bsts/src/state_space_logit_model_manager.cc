...
namespace BOOM {
  namespace bsts {
    ...
    namespace {
      using Manager = StateSpaceLogitModelManager;
    }  // namespace

    Manager::StateSpaceLogitModelManager()
        : predictor_dimension_(-1),
          clt_threshold_(5) {}

    StateSpaceLogitModel * Manager::CreateBareModel(
        SEXP r_data_list,
        SEXP r_prior,
        SEXP r_options,
    ...
    void Manager::SetPredictorDimension(int xdim) {
    ...
    Vector Manager::SimulateForecast(const Vector &final_state) {

...
namespace BOOM {
    ...
    class MixingWeightTargetFunction {
     public:
      ...
      double operator()(const Vector &parameters) const {
        ...
        Vector log_mixing_weights = log(multinomial_logit_inverse(
            ConstVectorView(parameters, 1)));
      ...
      double log_proposal_density(
        ...
        for (int i = 0; i < dim; ++i) {
          workspace[i] = log_mixing_weights[i]
  ...
  ///---------------------------------------------------------------------------
  AGMS::AdaptiveGaussianMixtureMhSampler(
  ...
  //---------------------------------------------------------------------------
  double AGMS::log_proposal_density(
      const Vector &x) const {
    Vector workspace = log_mixing_weights_;
    for (int i = 0; i < workspace.size(); ++i) {
      workspace[i] += dmvn(x, mixture_means_[i], shared_precision_, ldsi_, true);
    }
    return lse(workspace);
  }
  ...
  Vector AGMS::draw(const Vector &old) {
    ...
    double log_mh_ratio =
        candidate_log_density - candidate_log_proposal_density
  ...
  //---------------------------------------------------------------------------
  void AGMS::initialize_proposal_distribution(
  ...
  void AGMS::recompute_proposal_distribution_weights() {
    Vector parameters = concat(
        log_normalizing_constant_,
        multinomial_logit(mixing_weights_));

    MixingWeightTargetFunction target(candidates_,
                                      log_density_values_,
                                      shared_precision_,
                                      ldsi_,
                                      mixture_means_);

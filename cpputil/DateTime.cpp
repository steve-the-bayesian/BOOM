...
namespace BOOM{
  ...
  DateTime::DateTime(const Date &d, uint hour, uint min, double sec)
      : d_(d)
  {
    assert(hour < 24);
    assert(min < 60);
  ...
  DateTime::DateTime(double time_since_midnight_starting_jan_1_1970,
                     TimeScale scale)
  {
    double days = time_since_midnight_starting_jan_1_1970
  ...
  double DateTime::seconds_to_next_day()const{ return seconds_in_day_*(1 - t_) ;}
  ...
  double DateTime::seconds_left_in_hour()const{
    ...
    return fraction_of_hour > 0 ?
        seconds_in_an_hour * (1- (fraction_of_hour)) : 0;
  }

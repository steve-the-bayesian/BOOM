...
namespace BOOM{
  ...
  class Ptr<T, true> {  // intrusive pointers
   public:
    ...
    const boost::intrusive_ptr<T> & get_boost() const {return managed_pointer_;}
  ...
  class Ptr<T,false> {
   public:
    ...
    template<class Y> Ptr(Y* p) : managed_pointer_(p) {}
    template<class Y,class D> Ptr(Y* p, D d) : managed_pointer_(p,d) {}
    ~Ptr() {}

    Ptr(const Ptr &rhs): managed_pointer_(rhs.managed_pointer_) {}
    template <class Y> Ptr(const Ptr<Y> &rhs): managed_pointer_(rhs.get_boost()) {}
    template <class Y> Ptr(std::auto_ptr<Y> &rhs): managed_pointer_(rhs) {}
    ...
    template <class Y> inline void reset(Y *p) { managed_pointer_.reset(p);}
    template <class Y, class D> inline void reset(Y*p, D d) {
  ...
  inline
  D * get_deleter(const Ptr<T,false> &a) {
